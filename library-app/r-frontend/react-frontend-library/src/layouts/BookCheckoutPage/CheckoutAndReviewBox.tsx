    ...
    function buttonRender(){
        if(props.isAuthenticated){
            if(!props.isCheckedOut && props.currentLoansCount<5){
                return(<button onClick={()=>props.checkoutBook()} className="btn secondary-color btn-lg">Checkout</button>)
            }else if(props.isCheckedOut){
                return(<p className="secondary-color-text"><b>Book checked out. Enjoy!</b></p>)
            }else if(!props.isCheckedOut){
                return(<p className="text-danger">Too many
                books checked out</p>)
            }
        }
        return(<Link to={'/login'} className="btn secondary-color btn-lg">Sign in</Link>)
    }
        ...
        <div className={props.mobile? 'card d-flex mt-5' : 'card col-4 container d-flex mb-5'}>
            <div className="card-body container">
                <div className="mt-3">
                    ...
                    <h4 className="secondary-color-text">Available</h4>:
